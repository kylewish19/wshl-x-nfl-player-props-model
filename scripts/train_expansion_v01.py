...
d=load_raw()
frame=build_feature_frame(d["player_stats"],d.get("schedules"),d.get("pbp"),tuple(cfg["rolling_windows"]),auxiliary=d)
specs={
...
for prop,s in specs.items():
 m=train_prop_model(frame,prop,s["target"],s["positions"],s["kind"],
 ...
 save_model(m,root="models/expansion_v01")
 report[prop]={"algorithm":m.estimator_name,"feature_set":m.feature_set,**m.validation_metrics}
Path("reports").mkdir(exist_ok=True)
Path("reports/expansion_v0.1.0.json").write_text(json.dumps(report,indent=2))
